...
func (s *Server) GetSession(w http.ResponseWriter, r *http.Request, req *IdpAuthnRequest) *Session {
	...
	if r.Method == "POST" && r.PostForm.Get("user") != "" {
		...
		session := &Session{
			...
			ExpireTime: util.TimeNow().Add(sessionMaxAge),
	...
	if sessionCookie, err := r.Cookie("session"); err == nil {
		...
		if util.TimeNow().After(session.ExpireTime) {
			s.sendLoginForm(w, r, req, "")
			return nil
		}
		return session
...
func (s *Server) sendLoginForm(w http.ResponseWriter, _ *http.Request, req *IdpAuthnRequest, toast string) {
...
func (s *Server) HandleLogin(w http.ResponseWriter, r *http.Request) {
	...
	session := s.GetSession(w, r, &IdpAuthnRequest{IDP: &s.IDP})
	if session == nil {
		return
	}
...
func (s *Server) HandleGetSession(w http.ResponseWriter, r *http.Request) {
	session := Session{}
	err := s.Store.Get(fmt.Sprintf("/sessions/%s", mux.Vars(r)["id"]), &session)
	if err != nil {
		http.Error(w, http.StatusText(http.StatusInternalServerError), http.StatusInternalServerError)
		return
	}
...
func (s *Server) HandleDeleteSession(w http.ResponseWriter, r *http.Request) {
	err := s.Store.Delete(fmt.Sprintf("/sessions/%s", mux.Vars(r)["id"]))
	if err != nil {
		http.Error(w, http.StatusText(http.StatusInternalServerError), http.StatusInternalServerError)
		return
	}
