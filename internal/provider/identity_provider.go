...
func (idp *IdentityProvider) Metadata() *EntityDescriptor {
	...
	ed := &EntityDescriptor{
		...
		ValidUntil:    util.TimeNow().Add(validDuration),
		...
		IDPSSODescriptors: []IDPSSODescriptor{
			{
				SSODescriptor: SSODescriptor{
					...
					NameIDFormats: []format.NameID{format.NameID("urn:oasis:names:tc:SAML:2.0:nameid-format:transient")},
				},
...
func NewIdpAuthnRequest(idp *IdentityProvider, r *http.Request) (*IdpAuthnRequest, error) {
	...
	case "GET":
		...
		req.RequestBuffer, err = io.ReadAll(util.NewSaferFlateReader(bytes.NewReader(compressedRequest)))
		if err != nil {
