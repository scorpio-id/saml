...
func (m EntityDescriptor) MarshalXML(e *xml.Encoder, _ xml.StartElement) error {
	...
	aux := &struct {
		ValidUntil    format.RelaxedTime `xml:"validUntil,attr,omitempty"`
		CacheDuration format.Duration    `xml:"cacheDuration,attr,omitempty"`
		*Alias
	}{
		ValidUntil:    format.RelaxedTime(m.ValidUntil),
		CacheDuration: format.Duration(m.CacheDuration),
		Alias:         (*Alias)(&m),
	}
	return e.Encode(aux)
}
