...
func (r *LogoutRequest) Element() *etree.Element {
	...
	el.CreateAttr("IssueInstant", r.IssueInstant.Format(format.TimeFormat))
	if r.NotOnOrAfter != nil {
		el.CreateAttr("NotOnOrAfter", r.NotOnOrAfter.Format(format.TimeFormat))
	}
	if r.Destination != "" {
...
func (r *LogoutRequest) MarshalXML(e *xml.Encoder, _ xml.StartElement) error {
	...
	aux := &struct {
		IssueInstant format.RelaxedTime  `xml:",attr"`
		NotOnOrAfter *format.RelaxedTime `xml:",attr"`
		*Alias
	}{
		IssueInstant: format.RelaxedTime(r.IssueInstant),
		NotOnOrAfter: (*format.RelaxedTime)(r.NotOnOrAfter),
		Alias:        (*Alias)(r),
	}
	return e.Encode(aux)
}
...
func (a *Assertion) Element() *etree.Element {
	...
	el.CreateAttr("IssueInstant", a.IssueInstant.Format(format.TimeFormat))
...
func (s *SubjectConfirmationData) Element() *etree.Element {
	...
	if !s.NotBefore.IsZero() {
		el.CreateAttr("NotBefore", s.NotBefore.Format(format.TimeFormat))
	}
	if !s.NotOnOrAfter.IsZero() {
		el.CreateAttr("NotOnOrAfter", s.NotOnOrAfter.Format(format.TimeFormat))
	}
	if s.Recipient != "" {
...
func (s *SubjectConfirmationData) MarshalXML(e *xml.Encoder, start xml.StartElement) error {
	...
	aux := &struct {
		...
		*Alias
		...
		NotOnOrAfter: format.RelaxedTime(s.NotOnOrAfter),
		Alias:        (*Alias)(s),
	}
...
func (c *Conditions) Element() *etree.Element {
	...
	if !c.NotBefore.IsZero() {
		el.CreateAttr("NotBefore", c.NotBefore.Format(format.TimeFormat))
	}
	if !c.NotOnOrAfter.IsZero() {
		el.CreateAttr("NotOnOrAfter", c.NotOnOrAfter.Format(format.TimeFormat))
	}
...
func (c *Conditions) MarshalXML(e *xml.Encoder, start xml.StartElement) error {
	...
	aux := &struct {
		NotBefore    format.RelaxedTime `xml:",attr"`
		NotOnOrAfter format.RelaxedTime `xml:",attr"`
		*Alias
	}{
		NotBefore:    format.RelaxedTime(c.NotBefore),
		NotOnOrAfter: format.RelaxedTime(c.NotOnOrAfter),
		Alias:        (*Alias)(c),
	}
	return e.EncodeElement(aux, start)
}
...
func (a *AuthnStatement) Element() *etree.Element {
	...
	el.CreateAttr("AuthnInstant", a.AuthnInstant.Format(format.TimeFormat))
	if a.SessionIndex != "" {
		el.CreateAttr("SessionIndex", a.SessionIndex)
	}
	if a.SessionNotOnOrAfter != nil {
		el.CreateAttr("SessionNotOnOrAfter", a.SessionNotOnOrAfter.Format(format.TimeFormat))
	}
...
func (a *AuthnStatement) MarshalXML(e *xml.Encoder, start xml.StartElement) error {
	...
	aux := &struct {
		AuthnInstant        format.RelaxedTime  `xml:",attr"`
		SessionNotOnOrAfter *format.RelaxedTime `xml:",attr,omitempty"`
		*Alias
	}{
		AuthnInstant:        format.RelaxedTime(a.AuthnInstant),
		SessionNotOnOrAfter: (*format.RelaxedTime)(a.SessionNotOnOrAfter),
		Alias:               (*Alias)(a),
	}
	return e.EncodeElement(aux, start)
}
